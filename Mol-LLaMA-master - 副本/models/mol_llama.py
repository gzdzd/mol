...
class MolLLaMA(MolLLaMAPreTrainedModel):
    ...
    def forward(self, graph_batch, text_batch):
        _, _, query_output, stats = self.encoder.graph_forward(graph_batch)      
        ...
        inputs_embeds[text_batch.mol_token_flag] = \
            query_output.flatten(0, 1) # [batch_size, max_len, dim]
        ...
        # integrate lb_loss from blending stats into total loss if available
        if isinstance(stats, dict) and ("lb_loss" in stats):
            outputs.loss = outputs.loss + stats["lb_loss"].to(outputs.loss.device)
