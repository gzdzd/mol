...
class MolLLaMAEncoder(nn.Module):
    ...
    def compute_loss(self, graph_batch, text_batch):
        ...
        batch_node, batch_mask, query_output, stats = self.graph_forward(graph_batch)
        ...
        lb_loss = stats.get("lb_loss", torch.tensor(0.0, device=self.device)) if isinstance(stats, dict) else torch.tensor(0.0, device=self.device)

        return {
            "loss": loss_gtc + loss_gtm + loss_lm + lb_loss,
            "loss_gtc": loss_gtc,
            "loss_gtm": loss_gtm,
            "loss_lm": loss_lm,
            "lb_loss": lb_loss,
    ...
    def graph_forward(self, graph_batch):
        ...
        for encoder_type in self.encoder_types:
            ...
            # 对齐长度：Uni-Mol 输出包含 BOS/EOS 两个特殊位，切掉首尾
            if encoder_type == 'unimol':
        ...
        batch_node, batch_mask, stats = self.blending_module(batch_nodes, batch_masks)

        # 保持与 QFormer 权重一致的 dtype，避免 Float/BFloat16 混用
        q_dtype = next(self.Qformer.parameters()).dtype
        query_tokens = self.query_tokens.expand(batch_node.shape[0], -1, -1).to(q_dtype)
        batch_node = batch_node.to(q_dtype)
        ...
        return batch_node, batch_mask, query_output, stats
    ...
    def molecule_text_matching(self, sim_t2g, sim_g2t, batch_node, batch_mask,
        ...
        with torch.no_grad():
            # Stable weights: softmax -> mask diagonal -> sanitize -> renormalize
            weights_t2g = F.softmax(sim_t2g, dim=1)
            weights_g2t = F.softmax(sim_g2t, dim=1)
            # zero out diagonal (avoid sampling the positive pair)
            weights_t2g.fill_diagonal_(0)
            weights_g2t.fill_diagonal_(0)
            # sanitize any invalid values
            weights_t2g = torch.nan_to_num(weights_t2g, nan=0.0, posinf=0.0, neginf=0.0)
            weights_g2t = torch.nan_to_num(weights_g2t, nan=0.0, posinf=0.0, neginf=0.0)
            weights_t2g = torch.clamp(weights_t2g, min=0.0)
            weights_g2t = torch.clamp(weights_g2t, min=0.0)
            # renormalize rows; if a row sums to 0, fall back to uniform over non-diagonals
            B = weights_t2g.size(0)
            eye = torch.eye(B, device=weights_t2g.device, dtype=weights_t2g.dtype)
            uniform = (1.0 / max(B - 1, 1)) * (1 - eye)
            t2g_sum = weights_t2g.sum(dim=1, keepdim=True)
            g2t_sum = weights_g2t.sum(dim=1, keepdim=True)
            weights_t2g = torch.where(
                (t2g_sum <= 1e-12), uniform, weights_t2g / torch.clamp(t2g_sum, min=1e-12)
            )
            weights_g2t = torch.where(
                (g2t_sum <= 1e-12), uniform, weights_g2t / torch.clamp(g2t_sum, min=1e-12)
            )
