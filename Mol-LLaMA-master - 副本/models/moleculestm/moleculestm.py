...
from torch_geometric.utils import add_self_loops, softmax, degree, to_dense_batch
from torch_scatter import scatter_add
from models.moleculestm.mol_encoder import AtomEncoder, BondEncoder
...
class MoleculeSTM(nn.Module):
    ...
    def __init__(
        ...
        if molecule_node_model is not None:
            self.molecule_node_model = molecule_node_model
        else:
            self.molecule_node_model = GNN(num_layer, emb_dim, JK=JK, gnn_type='gin')
        self.num_layer = num_layer
    ...
    def forward(self, x, edge_index, edge_attr, batch, **unused):
        node_representation = self.molecule_node_model(x, edge_index, edge_attr) 
        graph_representation = self.pool(node_representation, batch) #(B, D)
        # output = self.graph_pred_linear(graph_representation)

        node_representation, node_mask = to_dense_batch(node_representation, batch) #(B, N, D)

        # 对齐到每原子序列，不拼接图级token，避免与Uni-Mol长度不一致
        return node_representation, node_mask.bool()