...
class Stage1Trainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        graph_batch, text_batch, iupac_names = batch
        self.scheduler.step(self.trainer.current_epoch, self.trainer.global_step)
        batch_size = text_batch.input_ids.size(0)
        loss = self.encoder.compute_loss(graph_batch, text_batch)
        ...
        if 'lb_loss' in loss:
            self.log("train_loss_lb", float(loss['lb_loss']), batch_size=batch_size, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if 'lb_loss' in loss:
            self.log("val_loss_lb", float(loss['lb_loss']), batch_size=batch_size, sync_dist=True)
